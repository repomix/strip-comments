...
const parse = (input, options = {}) => {
  ...
  const { LINE_REGEX, BLOCK_OPEN_REGEX, BLOCK_CLOSE_REGEX, BLOCK_REQUIRES_LINE_START } = lang;
  ...
  while (pos < len) {
    ...
    // quoted strings
    if (block.type !== 'block' && (!prev || !/\w$/.test(prev.value)) && !(tripleQuotes && input.startsWith('"""', pos))) {
    ...
    if (STICKY_BLOCK_OPEN_REGEX && options.block && canMatchBlockOpen && !(tripleQuotes && block.type === 'block')) {
    ...
    if (STICKY_BLOCK_CLOSE_REGEX && block.type === 'block' && options.block) {
      if ((token = scan(STICKY_BLOCK_CLOSE_REGEX, 'close'))) {
